...
class tsi_t : public htif_t
{
 ...
 protected:
  void reset() override;
  void read_chunk(addr_type taddr, size_t nbytes, void* dst) override;
  void write_chunk(addr_type taddr, size_t nbytes, const void* src) override;
  void switch_to_target();

  size_t chunk_align() override { return 4; }
  size_t chunk_max_size() override { return 1024; }

  int get_ipi_addrs(addr_type *addrs);
 ...
 private:
  ...
  void push_addr(addr_type addr);
  void push_len(addr_type len);
