...
void htif_t::load_program()
{
  ...
  if(symbols.count("sc_exit")) {
    sc_exit_addr = symbols["sc_exit"];
    fprintf(stderr, "sc_exit: %p\n", sc_exit_addr);
  }

  if(symbols.count("xreg_output_data")) {
    xreg_output_data_addr = symbols["xreg_output_data"];
    fprintf(stderr, "xreg_output_data %p\n", xreg_output_data_addr);
  }

  if(symbols.count("freg_output_data")) {
    freg_output_data_addr = symbols["freg_output_data"];
    fprintf(stderr, "freg_output_data: %p\n", freg_output_data_addr);
  }
}

uint64_t htif_t::get_sc_exit_addr()
{
  return sc_exit_addr;
}

uint64_t htif_t::get_xreg_output_data()
{
  return xreg_output_data_addr;
}

uint64_t htif_t::get_freg_output_data()
{
  return freg_output_data_addr;
}
